...
CREATE POLICY "users_own_records" ON users FOR ALL USING (auth.uid() = id);
CREATE POLICY "word_lists_owner_access" ON word_lists FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "words_list_owner_access" ON words FOR ALL USING (list_id IN (SELECT id FROM word_lists WHERE user_id = auth.uid()));
CREATE POLICY "word_progress_own_records" ON word_progress FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "puzzle_sessions_own_records" ON puzzle_sessions FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "word_reviews_own_records" ON word_reviews FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "puzzle_cache_own_records" ON puzzle_cache FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "authenticated_view_shared_lists" ON shared_lists FOR SELECT TO authenticated USING (is_active = true AND (created_by = auth.uid() OR id IN (SELECT shared_list_id FROM list_collaborators WHERE user_id = auth.uid())));
CREATE POLICY "shared_lists_owner_manage" ON shared_lists FOR ALL USING (auth.uid() = created_by);
CREATE POLICY "list_collaborators_member_access" ON list_collaborators FOR ALL USING (user_id = auth.uid() OR shared_list_id IN (SELECT id FROM shared_lists WHERE created_by = auth.uid()));
CREATE POLICY "webhook_events_service_only" ON stripe_webhook_events FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "daily_streaks_own_records" ON daily_streaks FOR ALL USING (auth.uid() = user_id);
...
DROP FUNCTION IF EXISTS leave_collaborative_list(UUID);

CREATE OR REPLACE FUNCTION calculate_srs_progress(user_uuid UUID)
RETURNS TABLE(total_words BIGINT, learned_words BIGINT, review_due BIGINT, success_rate DECIMAL) AS $$
BEGIN
  RETURN QUERY
  SELECT
    ...
    COALESCE(ROUND((SUM(CASE WHEN wp.total_reviews > 0 THEN wp.correct_reviews::DECIMAL / wp.total_reviews ELSE 0 END) / GREATEST(COUNT(DISTINCT CASE WHEN wp.total_reviews > 0 THEN w.id END), 1)) * 100, 2), 0) as success_rate
  FROM words w
  LEFT JOIN word_progress wp ON w.id = wp.word_id AND wp.user_id = user_uuid
  WHERE w.list_id IN (SELECT id FROM word_lists WHERE user_id = user_uuid);
END;
...
BEGIN
  ...
  WHERE sl.share_token = share_token_param AND sl.is_active = true;
  ...
  FROM words WHERE list_id = shared_list_record.word_list_id;

  GET DIAGNOSTICS copied_word_count = ROW_COUNT;

  RETURN json_build_object('success', true, 'list_id', new_list_id, 'word_count', copied_word_count, 'message', format('Successfully imported %s words', copied_word_count));
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
  ...
  FROM shared_lists sl
  WHERE sl.share_token = share_token_param AND sl.is_active = true AND sl.is_collaborative = true;
  ...
  RETURN json_build_object('success', true, 'list_id', shared_list_record.word_list_id, 'message', 'Successfully joined collaborative list');
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
  ...
  WHERE shared_list_id IN (SELECT id FROM shared_lists WHERE word_list_id = list_id_param) AND user_id = auth.uid();
...
CREATE TRIGGER update_daily_streaks_updated_at BEFORE UPDATE ON daily_streaks FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
FROM word_lists wl WHERE wl.name = 'Sample Vocabulary'
