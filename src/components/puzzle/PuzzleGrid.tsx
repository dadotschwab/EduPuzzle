...
export const PuzzleGrid = memo(function PuzzleGrid({
  ...
  const [internalFocusedCell, setInternalFocusedCell] = useState<{ x: number; y: number } | null>(
    null
  )
  ...
  const focusedCell =
    controlledFocusedCell !== undefined ? controlledFocusedCell : internalFocusedCell
  ...
  const getCellColorClass = useCallback(
    ...
    [cellStatusMap, isPuzzleCompleted, showCorrectAnswers]
  )
   ...
   */
  const isCellInSelectedWord = useCallback(
  ...
  const getWordAtCell = useCallback(
    (x: number, y: number, preferDirection?: 'horizontal' | 'vertical'): PlacedWord | null => {
  ...
  const getNextWord = useCallback(
    (currentWord: PlacedWord): PlacedWord | null => {
   ...
   */
  const moveToNextCellInWord = useCallback(
    (currentX: number, currentY: number, justTypedLetter?: string) => {
    ...
    [activeWord, userInput, getNextWord, onWordSelect, setFocusedCell]
  )
  ...
  const moveToAdjacentCell = useCallback(
    (currentX: number, currentY: number, direction: 'up' | 'down' | 'left' | 'right') => {
      let newX = currentX
      let newY = currentY
    ...
    [puzzle.gridSize, puzzle.grid, activeWord, getWordAtCell, onWordSelect, setFocusedCell]
  )
  ...
  }, [selectedWord, getNextWord, onWordSelect, setFocusedCell])
  ...
  const handleCellClick = useCallback(
    ...
    [selectedWord, isCellInSelectedWord, getWordAtCell, activeWord, onWordSelect, setFocusedCell]
  )

  return (
    <div ref={gridRef} className="w-full aspect-square max-w-2xl mx-auto">
      <div
      ...
      >
        {Array.from({ length: puzzle.gridSize }).map((_, y) =>
          Array.from({ length: puzzle.gridSize }).map((_, x) => {
            ...
            if (isBlackSquare) {
              return <div key={cellKey} className="bg-gray-900 rounded-sm aspect-square" />
            }
