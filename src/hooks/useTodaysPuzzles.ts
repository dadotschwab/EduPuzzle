 ...
 *
 * Strategy:
 * 1. Group all words by language pair
 * 2. Within each language pair, separate by list size
 * 3. Large lists (≥15 words) get their own puzzle
 * 4. Small lists (<15 words) are combined if total ≥ MIN_WORDS
 */
async function smartGroupWords(dueWords: WordWithProgress[]): Promise<PuzzleGroup[]> {
  const groups: PuzzleGroup[] = []

  // Group by language pair first
