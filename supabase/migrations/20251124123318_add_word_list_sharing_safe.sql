...
CREATE POLICY IF NOT EXISTS "owners_manage_shared_lists"
  ON public.shared_lists
  FOR ALL
  USING (auth.uid() = created_by);

-- Anyone can view active shared lists (for token validation)
CREATE POLICY IF NOT EXISTS "anyone_view_active_shared_lists"
  ON public.shared_lists
  FOR SELECT
...
CREATE POLICY IF NOT EXISTS "owners_manage_collaborators"
...
CREATE POLICY IF NOT EXISTS "collaborators_view_own_membership"
  ON public.list_collaborators
  FOR SELECT
...
CREATE POLICY IF NOT EXISTS "collaborators_view_shared_words"
  ON public.words
  FOR SELECT
  USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "collaborators_modify_shared_words"
  ON public.words
  FOR ALL
  USING (
    EXISTS (
