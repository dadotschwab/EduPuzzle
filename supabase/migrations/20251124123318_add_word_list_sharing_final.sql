...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE public.word_lists ADD COLUMN is_shared BOOLEAN DEFAULT false;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE public.word_lists ADD COLUMN shared_at TIMESTAMPTZ;
  END IF;
END $$;
...
-- Drop existing policies if they exist (safe for Supabase)
DO $$
BEGIN
  -- Drop policies if they exist
  IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'owners_manage_shared_lists' AND tablename = 'shared_lists') THEN
    EXECUTE 'DROP POLICY "owners_manage_shared_lists" ON public.shared_lists';
  END IF;

  IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'anyone_view_active_shared_lists' AND tablename = 'shared_lists') THEN
    EXECUTE 'DROP POLICY "anyone_view_active_shared_lists" ON public.shared_lists';
  END IF;

  IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'owners_manage_collaborators' AND tablename = 'list_collaborators') THEN
    EXECUTE 'DROP POLICY "owners_manage_collaborators" ON public.list_collaborators';
  END IF;

  IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'collaborators_view_own_membership' AND tablename = 'list_collaborators') THEN
    EXECUTE 'DROP POLICY "collaborators_view_own_membership" ON public.list_collaborators';
  END IF;

  IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'collaborators_view_shared_words' AND tablename = 'words') THEN
    EXECUTE 'DROP POLICY "collaborators_view_shared_words" ON public.words';
  END IF;

  IF EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'collaborators_modify_shared_words' AND tablename = 'words') THEN
    EXECUTE 'DROP POLICY "collaborators_modify_shared_words" ON public.words';
  END IF;
END $$;
